...
def login(req: LoginReq):
    ...
    conn = get_conn() 
    conn.execute("INSERT INTO tokens (token, username) VALUES (?, ?)", (token, req.username))
    conn.commit()
    conn.close()
...
def get_current_user(authorization: str = Header(default="")) -> str:
    ...
    print(f"🔑 验票: token={token}, 用户={row['username'] if row else None}")
    conn.close()
    if row is None:
        raise HTTPException(status_code=401, detail="Token 无效或已过期")
    return row["username"]  # 返回值会被注入到接口函数的参数里！
